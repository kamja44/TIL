...
- 무선 네트워크를 찾기 위해 무선 장치를 갖고 주위의 AP(Access Point)를 찾는 과정을 말한다.
...
```
access-list acl번호 [permit(허용) 또는 deny(차단)] 프로토콜 출발지 [출발지-wildcard] 목적지 [목적지-wildcard] [목적지 포트]
```

예

1. access-list 150 deny tcp 192.168.1.0 0.0.0.255 host 100.100.100.1 eq 80

- 출발지 IP가 192.168.1.0/24 대역이고, 목적지 IP가 100.100.100.1(특정 IP 지정시 host 키워드명시)이고, 목적지 포트가 80(eq는 equal을 의미한다.)인 TCP 패킷에 대해 모두 차단(필터링)한다.

2. access-list 150 deny udp host 192.168.2.5 100.100.100.0 0.0.0.255 range 100 200

- 출발지 IP가 192.168.2.5이고, 목적지 IP가 100.100.100.0/24 대역이고, 목적지 포트가 100 ~ 200 (range를 통해 포트 범위 지정)인 UDP 패킷에 대해 모두 차단(필터링)한다.

3. access-list 100 deny udp any any eq 53

- acl 번호를 100으로 설정하고, 출발지 및 목적지가 모두 any(모든 패킷을 의미)이고, 목적지 포트가 53인 UDP 패킷을 차단(필터링)한다.

# SNMP(Simple Network Management Protocol)

- `TCP/IP 기반 네트워크상의 각 호스트에서 정기적으로 여러 정보를 자동으로 수집하여 네트워크 관리`를 하기 위한 프로토콜이다.
- 라우터에서 SNMP 서비스를 비활성화 하기 위해선 다음과 같이 입력한다.
...
# 라우터에서 directed broadcast 허용하지 않게 설정

- 라우터에서 directed broadcast 설정을 허용하면, 내부 네트워크가 여러 공격에 대한 증폭과 반사 수단으로 악용될 수 있다.
...
- 관리자 모드에서 설정 모드로 전환한다.

Router(config)# interface FastEthernet 0/0

- 설정할 인터페이스(FastEthernet 0/0)를 지정한다.

Router(config-if)# no IP directed-broadcast

- ip directed-broadcast를 허용하지 않는다.

# 라우팅

- 정적 또는 동적 방식을 사요하여, 라우팅테이블을 갱신하고 이를 통해 전송할 패킷의 최적 목적지 경로를 선정하여 전송한다.

## 정적 라우팅(static routing)

- `관리자가 직접 라우팅 테이블을 설정`하는 방식이다.
- 관리자가 설정해준 경로로만 전달된다.
  - 라우터 간의 정보 교환이 필요하지 않아 트래픽의 낭비를 막을 수 있다.

## 동적 라우팅

- `라우팅 프로토콜을 이용하여 자동으로 경로를 탐색하고 패킷을 전달`하는 방법이다.

## 동적 라우팅 프로토콜 분류

- IGP(Interior Gateway Protocol)
  - 자율 시스템(AS: Autonomous System) 내부에서 사용하는 라우팅 프로토콜이다.
    - 자율 시스템은 조직/기관 내부에서 직접 관리하는 라우터의 집합을 말한다.
  - 거리 벡터(distance vector) 방식
    - RIP, RIPv2, IGRP
  - 링크 상태(link state) 방식
    - IS-IS, OSPF
  - 하이브리드(hybrid, 거리 벡터와 링크 상태 방식 결합)
    - EIGRP
- EGP(External Gateway Protocol)
  - 자율 시스템(AS: Autonomous System) 간에 사용하는 라우팅 프로토콜이다.
